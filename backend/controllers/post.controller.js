    ...
    const eventsWithAllPosts = await Promise.all(
      events.map(async (event) => {
        ...
        if (req.user.role === "manager") {
            // Manager chỉ duyệt bài của User
            filteredPosts = posts.filter(
                (post) => post.userId.role === "user"
            );
        }

        if (req.user.role === "admin") {
            // Admin chỉ duyệt bài của Manager
            filteredPosts = posts.filter(
                (post) => post.userId.role === "manager"
            );
        }
...
const getAllPost = async (req, res) => {
  try {
    let posts = await Post.find({status: "pending"})
      .populate("userId", "name avatar role")
      .populate("eventId");

    // Admin chỉ thấy bài của Manager
    if (req.user.role === "admin") {
      posts = posts.filter(post => post.userId?.role === "manager");
    }

    // Manager chỉ thấy bài của User
    if (req.user.role === "manager") {
      posts = posts.filter(post => post.userId?.role === "user");
    }

    return res.status(200).json({
      success: true,
      message: "Lấy danh sách tất cả bài đăng",
      posts,
    });
  } catch (error) {
    return res.status(500).json({
