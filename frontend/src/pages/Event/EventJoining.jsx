    ...
    <>
      <div className="flex flex-col gap-6">
        <div className="pb-4 border-b border-slate-100 flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">Sự kiện đang tham gia</h1>
            <p className="text-xs text-slate-500 mt-1">Danh sách các hoạt động thiện nguyện bạn đang là thành viên chính thức</p>
          </div>
          <span className="text-xs font-bold px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-100">
            {events.length} Đang tham gia
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {events.length === 0 ? (
            <div className="col-span-full py-16 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-200">
              <p className="font-bold text-slate-700 text-base">Bạn chưa tham gia sự kiện nào</p>
              <p className="text-xs text-slate-400 mt-1">Hãy khám phá các sự kiện mới và gửi đăng ký tham gia ngay!</p>
            </div>
          ) : (
            events
              .filter((ev) => ev)
