    ...
    <>
      <div className="flex flex-col gap-6">
        <div className="pb-4 border-b border-slate-100 flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">Sự kiện đã được duyệt</h1>
            <p className="text-xs text-slate-500 mt-1">Danh sách các chiến dịch đã thông qua kiểm duyệt và đang hoạt động</p>
          </div>
          <span className="text-xs font-bold px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-100">
            {events.length} Chiến dịch
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {events.length === 0 ? (
            <div className="col-span-full py-16 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-200">
              <p className="font-bold text-slate-700 text-base">Chưa có sự kiện nào được duyệt</p>
              <p className="text-xs text-slate-400 mt-1">Các sự kiện sau khi gửi phê duyệt thành công sẽ xuất hiện tại đây.</p>
            </div>
          ) : (
            events
              .filter((ev) => ev)
              .map((event) => (
                ...
                >
                  ...
                  <div className="relative w-full h-[180px] overflow-hidden bg-slate-100">
                    ...
                    <span className="absolute top-3 right-3 bg-emerald-600/90 backdrop-blur-md text-white text-[11px] font-semibold px-3 py-1 rounded-full shadow-sm">
                      ✓ Đã duyệt
                    </span>
                  </div>
